...
import api, { LoyaltyProgram as ApiLoyaltyProgram, LoyaltyStats as ApiLoyaltyStats, LoyaltyReward as ApiLoyaltyReward } from '@/lib/api';
...
export function GestoreLoyaltyProgramScreen({ onBack }: GestoreLoyaltyProgramScreenProps) {
  ...
  const loadData = async () => {
    try {
      ...
      const apiStats = await api.getLoyaltyStats();
      // Note: LoyaltyPrograms API available but levels are local config

      setStats({
        totalMembers: apiStats.totalMembers,
        activeMembers: apiStats.activeMembers,
        totalPointsIssued: apiStats.pointsIssued,
        totalPointsRedeemed: apiStats.pointsRedeemed,
        pointsPerPurchase: 1,
        euroPerPoint: 0.01,
      });

      // Levels are local config - API doesn't provide tier breakdown
      // Member distribution is estimated from total
      if (apiStats.totalMembers > 0) {
        setLevels([
          {
            id: 'bronze',
            name: 'Bronze',
            ...
            color: '#CD7F32',
            icon: 'shield',
            membersCount: Math.floor(apiStats.totalMembers * 0.5),
          },
          {
            id: 'silver',
            name: 'Silver',
            minPoints: 1000,
            maxPoints: 4999,
            benefits: ['Sconto 10% su tutti gli eventi', 'Accesso anticipato vendite'],
            color: '#C0C0C0',
            icon: 'shield',
            membersCount: Math.floor(apiStats.totalMembers * 0.25),
          },
          {
            id: 'gold',
            name: 'Gold',
            minPoints: 5000,
            maxPoints: 14999,
            benefits: ['Sconto 15% su tutti gli eventi', 'Accesso VIP', 'Priority check-in'],
            color: '#FFD700',
            icon: 'shield',
            membersCount: Math.floor(apiStats.totalMembers * 0.15),
          },
          {
            id: 'platinum',
            name: 'Platinum',
            minPoints: 15000,
            maxPoints: null,
            benefits: ['Sconto 25%', 'Area VIP', 'Concierge dedicato', 'Eventi esclusivi'],
            color: '#E5E4E2',
            icon: 'diamond',
            membersCount: Math.floor(apiStats.totalMembers * 0.05),
          },
        ]);
      } else {
        setLevels([]);
      ...
      // Rewards require separate API call per program, not loaded here
      setRewards([]);
    } catch (error) {
