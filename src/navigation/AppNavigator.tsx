...
import { BottomTabNavigator, TabName } from '@/components/BottomTabNavigator';
import { PRBottomTabNavigator, PRTabName } from '@/components/PRBottomTabNavigator';
import { ScannerBottomTabNavigator, ScannerTabName } from '@/components/ScannerBottomTabNavigator';
...
import { PRTrackingLinksScreen } from '@/screens/pr/PRTrackingLinksScreen';
...
type Screen =
  ...
  | { name: 'gestoreEventHub'; params: { eventId: string } }
...
export function AppNavigator() {
  ...
  const renderScreen = () => {
    switch (currentScreen.name) {
      ...
      case 'landing':
        return (
          <LandingScreen
            ...
            onNavigateEventDetail={(eventId) => navigate({ name: 'eventDetail', params: { eventId } })}
      ...
      case 'prDashboard':
        return (
          <PRDashboard
            ...
            onNavigateTrackingLinks={() => navigate({ name: 'prTrackingLinks' })}
      ...
      case 'prTrackingLinks':
        return (
          <PRTrackingLinksScreen
            onBack={goBack}
          />
        );
      ...
      case 'gestoreDashboard':
        return (
          <GestoreDashboard
            ...
            onNavigateLocations={() => navigate({ name: 'gestoreLocations' })}
            onLogout={() => resetTo({ name: 'landing' })}
          />
        );
  ...
  const clientTabScreens: Screen['name'][] = [
    'landing', 'events', 'tickets', 'accountDashboard',
  ];

  const isClientTabScreen = clientTabScreens.includes(currentScreen.name);
  ...
  const getActiveTab = (): TabName => {
    switch (currentScreen.name) {
      ...
      case 'accountDashboard': return 'account';
      default: return 'home';
    }
  };
  ...
  const prTabScreens: Screen['name'][] = [
    'prDashboard', 'prEvents', 'prWallet', 'prProfile',
  ];

  const isPrTabScreen = prTabScreens.includes(currentScreen.name);

  const getPrActiveTab = (): PRTabName => {
    switch (currentScreen.name) {
      case 'prDashboard': return 'home';
      case 'prEvents': return 'events';
      case 'prWallet': return 'wallet';
      case 'prProfile': return 'profile';
      default: return 'home';
    }
  };

  const handlePrTabPress = (tab: PRTabName) => {
    switch (tab) {
      case 'home':
        resetTo({ name: 'prDashboard' });
        break;
      case 'events':
        resetTo({ name: 'prEvents' });
        break;
      case 'wallet':
        resetTo({ name: 'prWallet' });
        break;
      case 'profile':
        resetTo({ name: 'prProfile' });
        break;
    }
  };

  const scannerTabScreens: Screen['name'][] = [
    'scannerDashboard', 'scannerEvents', 'scannerProfile',
  ];

  const isScannerTabScreen = scannerTabScreens.includes(currentScreen.name);

  const getScannerActiveTab = (): ScannerTabName => {
    switch (currentScreen.name) {
      case 'scannerDashboard': return 'home';
      case 'scannerEvents': return 'events';
      case 'scannerProfile': return 'profile';
      default: return 'home';
    }
  };

  const handleScannerTabPress = (tab: ScannerTabName) => {
    switch (tab) {
      case 'home':
        resetTo({ name: 'scannerDashboard' });
        break;
      case 'events':
        resetTo({ name: 'scannerEvents' });
        break;
      case 'profile':
        resetTo({ name: 'scannerProfile' });
        break;
    }
  };
  ...
  return (
    <CartContext.Provider value={cartContextValue}>
      <View style={styles.container}>
        ...
        {isPrTabScreen && (
          <PRBottomTabNavigator
            activeTab={getPrActiveTab()}
            onTabPress={handlePrTabPress}
          />
        )}
        {isScannerTabScreen && (
          <ScannerBottomTabNavigator
            activeTab={getScannerActiveTab()}
            onTabPress={handleScannerTabPress}
          />
        )}
      </View>
