...
import { View, Text, StyleSheet, ScrollView, Pressable, TextInput, Alert, Modal, KeyboardAvoidingView, Platform, Image, ActivityIndicator } from 'react-native';
...
export function PRProfileScreen({ onGoBack, onLogout }: PRProfileScreenProps) {
  ...
  const getImageFullUrl = (path: string | null | undefined): string | null => {
    if (!path) return null;
    if (path.startsWith('http')) return path;
    const cleanPath = path.replace(/^\/api\/objects/, '').replace(/^\.private\//, '');
  ...
  const loadProfile = async () => {
    try {
      setIsLoading(true);
      const [profileData, pageSettingsData] = await Promise.all([
        api.getPrProfile(),
        api.getPrPageSettings().catch(() => null),
      ]);
      setProfile(profileData);
      setDisplayName(profileData?.displayName || '');
      setEmail(profileData?.email || '');
      if (pageSettingsData?.profileImageUrl) {
        setProfileImageUrl(getImageFullUrl(pageSettingsData.profileImageUrl));
      }
    } catch (error) {
  ...
  const uploadProfileImage = async (uri: string) => {
    ...
    try {
      const filename = `pr_profile_${profile?.prCode}_${Date.now()}.jpg`;
      const objectPath = `.private/pr-profiles/${filename}`;
      ...
      await api.updatePrPageSettings({ profileImageUrl: imageUrl });

      setProfileImageUrl(getImageFullUrl(imageUrl));
    ...
    Alert.alert(
      ...
      [
        { text: 'Annulla', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await api.updatePrPageSettings({ profileImageUrl: null });
              setProfileImageUrl(null);
              triggerHaptic('success');
              Alert.alert('Fatto', 'Foto profilo rimossa');
            } catch {
              triggerHaptic('error');
      ...
      >
        <View style={styles.avatarSection}>
          <Pressable
            ...
            onLongPress={profileImageUrl ? handleRemoveProfileImage : undefined}
            disabled={imageUploading}
            testID="button-pick-profile-image"
          ...
          <Text style={styles.imageHint}>
            {imageUploading ? 'Caricamento...' : 'Tocca per cambiare foto'}
          </Text>
