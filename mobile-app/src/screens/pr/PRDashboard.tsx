...
export function PRDashboard({
  ...
  const loadData = async () => {
    try {
      ...
      const now = new Date();
      const activeEvents = eventsData.filter((e: PrEvent) => new Date(e.eventEnd || e.eventStart) >= now);
      activeEvents.sort((a: PrEvent, b: PrEvent) => new Date(a.eventStart).getTime() - new Date(b.eventStart).getTime());
  ...
  const getActionConfig = (actionId: PrQuickAction) => {
    const configs: Record<PrQuickAction, { icon: keyof typeof Ionicons.glyphMap; label: string; gradient: 'golden' | 'teal' | 'purple' | 'blue' | 'pink'; onPress: () => void }> = {
      ...
      'tracking': { icon: 'link', label: 'Link', gradient: 'teal', onPress: onNavigateTrackingLinks },
