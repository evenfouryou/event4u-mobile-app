...
export function GestoreMarketingDashboardScreen({ onBack, onNavigate }: GestoreMarketingDashboardScreenProps) {
  ...
  const loadData = async () => {
    try {
      ...
      const [apiStats, apiCampaigns] = await Promise.all([
        api.getGestoreMarketingStats(),
        api.getGestoreCampaigns(),
      ]);

      setStats({
        totalCampaigns: apiStats.totalCampaigns,
        activeCampaigns: apiStats.activeCampaigns,
        scheduledCampaigns: 0,
        totalSent: apiStats.totalEmails,
        totalOpened: Math.round(apiStats.totalEmails * (apiStats.openRate / 100)),
        totalClicked: 0,
        openRate: apiStats.openRate,
        clickRate: 0,
      });
      // Note: scheduledCampaigns, totalClicked, clickRate not provided by API

      setCampaigns(apiCampaigns.map(c => ({
        id: c.id,
        name: c.name,
        type: c.type as 'email' | 'push' | 'sms',
        status: c.status as 'active' | 'scheduled' | 'completed' | 'draft',
                ...
                {performanceData.length > 0 ? (
                  ...
                  <View style={styles.emptyState}>
                    ...
                    <Text style={[styles.emptyStateText, { color: colors.mutedForeground }]}>
                      Dati di performance non disponibili
                    </Text>
                  </View>
                )}
              </Card>
