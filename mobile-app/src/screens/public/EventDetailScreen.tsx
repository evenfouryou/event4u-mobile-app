...
export function EventDetailScreen({
  ...
  const event = eventData ? {
    ...
    sectors: (eventData.sectors || []).map((s: any) => {
      ...
      const priceIntero = Number(s.priceIntero) || 0;
      const priceRidotto = Number(s.priceRidotto) || 0;
      const available = s.availableSeats ?? s.capacity ?? 0;
      ...
      if (ticketTypes.length === 0) {
        ticketTypes.push({
          ...
          name: 'Intero',
          price: priceIntero,
          available: priceIntero > 0 || priceRidotto > 0 ? available : 0,
          type: 'intero',
        });
      }
  ...
  const handleAddToCart = () => {
    if (selectedSector && selectedTicketType && event && eventData) {
      const sector = event.sectors.find(s => s.id === selectedSector);
      const ticket = sector?.ticketTypes.find(t => t.id === selectedTicketType);
      if (sector && ticket) {
        triggerHaptic('success');
        addToCart({
          ticketedEventId: eventData.id,
          eventName: event.name,
          eventDate: eventData.eventStart || new Date().toISOString(),
          sectorId: sector.id,
