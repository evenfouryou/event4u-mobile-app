...
class ApiClient {
  ...
  async request<T>(endpoint: string, options: RequestOptions = {}, isRetry: boolean = false): Promise<T> {
    ...
    if (this.authToken) {
      ...
      console.log(`[API] ${method} ${endpoint} - Auth header set: Bearer ${this.authToken.substring(0, 15)}...`);
    } else {
      console.log(`[API] ${method} ${endpoint} - NO AUTH TOKEN`);
    }
  ...
  async getPublicVenues(params?: { limit?: number; city?: string; userLat?: number; userLng?: number }): Promise<PublicVenue[]> {
    const parts: string[] = [];
    if (params?.limit) parts.push(`limit=${params.limit}`);
    ...
    if (params?.userLng) parts.push(`userLng=${params.userLng}`);
    const query = parts.length > 0 ? `?${parts.join('&')}` : '';
    return this.get<PublicVenue[]>(`/api/public/venues${query}`, { cacheTTL: 5 * 60 * 1000 });
  }
  ...
  async getPrPageSettings(): Promise<{
    prCode: string;
    displayName: string | null;
    firstName: string;
    lastName: string;
    ...
    return this.get('/api/pr/page-settings');
  }

  async updatePrPageSettings(data: {
    pageEnabled?: boolean;
    bio?: string;
    displayName?: string;
    profileImageUrl?: string | null;
    ...
    return this.put('/api/pr/page-settings', data);
  }

  async getPrTrackingLinks(): Promise<{
    prCode: string;
    links: Array<{
      eventId: string;
      eventName: string;
    ...
    return this.get('/api/pr/tracking-links');
  }
  ...
  async getPrEvents(): Promise<PrEvent[]> {
    console.log('[API] getPrEvents - authToken:', this.authToken ? `${this.authToken.substring(0, 15)}...` : 'NULL');
    const events = await this.get<any[]>('/api/pr/my-events');
    console.log('[API] getPrEvents - success, got', events?.length || 0, 'events');
  ...
  async getPrEventTables(eventId: string): Promise<PrEventTable[]> {
    try {
      const result = await this.get<any>(`/api/pr/events/${eventId}/table-types`);
      const types = Array.isArray(result) ? result : [];
      return types.map((t: any) => ({
        id: t.id,
        name: t.name,
        price: t.price || null,
        maxGuests: t.maxGuests || 0,
        totalQuantity: t.totalQuantity || 0,
        bookedCount: t.bookedCount || 0,
  ...
  async getPrTableReservations(eventId: string): Promise<any[]> {
    return this.get<any[]>(`/api/pr/events/${eventId}/table-reservations`);
  }

  async bookPrTable(eventId: string, data: { tableTypeId: string; customerName: string; customerPhone?: string; guests?: Array<{ firstName: string; lastName: string; phone?: string; gender?: string }>; notes?: string }): Promise<any> {
    return this.post<any>(`/api/pr/events/${eventId}/table-reservations`, {
  ...
  async getGestoreEvents(): Promise<GestoreEvent[]> {
    try {
      console.log('[API] getGestoreEvents - calling /api/gestore/events');
      console.log('[API] getGestoreEvents - authToken:', this.authToken ? `${this.authToken.substring(0, 15)}...` : 'NULL');
      const result = await this.get<GestoreEvent[]>('/api/gestore/events');
      console.log('[API] getGestoreEvents - got', result?.length || 0, 'events');
      return result;
    } catch (error) {
      console.error('[API] getGestoreEvents - error:', error);
      return [];
    }
  }
  ...
  async getUserFeatures(): Promise<UserFeatures> {
    try {
      return await this.get<UserFeatures>('/api/user-features/current/my', { cacheTTL: 300000 });
    } catch {
      return {
        userId: '',
  ...
  async getCompanyFeatures(): Promise<CompanyFeatures> {
    try {
      return await this.get<CompanyFeatures>('/api/company-features/current/my', { cacheTTL: 300000 });
    } catch {
      return {
        companyId: '',
        beverageEnabled: true,
  ...
  async updateGestoreEvent(eventId: string, data: any): Promise<{ id: string }> {
    return this.put<{ id: string }>(`/api/gestore/events/${eventId}`, data);
  }
  ...
  async getGestorePRs(): Promise<GestorePR[]> {
    try {
      return await this.get<GestorePR[]>('/api/reservations/pr-profiles?includeStaff=true');
    } catch {
      return [];
    }
  }

  async searchUsersForPR(phone: string): Promise<SearchedUser[]> {
    try {
      return await this.get<SearchedUser[]>(`/api/reservations/search-users?phone=${encodeURIComponent(phone)}`);
    } catch {
      return [];
    }
  }

  async createPR(data: CreatePRData): Promise<GestorePR> {
    return this.post<GestorePR>('/api/reservations/pr-profiles', data);
  }

  async updatePR(id: string, data: UpdatePRData): Promise<GestorePR> {
    return this.patch<GestorePR>(`/api/reservations/pr-profiles/${id}`, data);
  }

  async deactivatePR(id: string): Promise<void> {
    return this.delete(`/api/reservations/pr-profiles/${id}`);
  }

  async resendPRSms(id: string): Promise<{ success: boolean }> {
    return this.post<{ success: boolean }>(`/api/reservations/pr-profiles/${id}/resend-sms`, {});
  }

  async togglePRStatus(id: string, isActive: boolean): Promise<GestorePR> {
    return this.patch<GestorePR>(`/api/reservations/pr-profiles/${id}`, { isActive });
  }
  ...
  }): Promise<GestoreLocation> {
    return this.post<GestoreLocation>('/api/locations', data);
  }
  ...
  async updateLocation(locationId: string, data: {
    ...
    return this.patch<GestoreLocation>(`/api/locations/${locationId}`, data);
  }
  ...
  async getFixedCosts(): Promise<FixedCost[]> {
    try {
      return await this.get<FixedCost[]>('/api/fixed-costs');
    } catch {
      return [];
    }
  }

  async createFixedCost(data: Partial<FixedCost>): Promise<FixedCost> {
    return this.post<FixedCost>('/api/fixed-costs', data);
  }

  async updateFixedCost(id: string, data: Partial<FixedCost>): Promise<FixedCost> {
    return this.patch<FixedCost>(`/api/fixed-costs/${id}`, data);
  }

  async deleteFixedCost(id: string): Promise<void> {
    await this.delete(`/api/fixed-costs/${id}`);
  }

  // Extra Costs CRUD
  async getExtraCosts(): Promise<ExtraCost[]> {
    try {
      return await this.get<ExtraCost[]>('/api/extra-costs');
    } catch {
      return [];
    }
  }

  async createExtraCost(data: Partial<ExtraCost>): Promise<ExtraCost> {
    return this.post<ExtraCost>('/api/extra-costs', data);
  }

  async updateExtraCost(id: string, data: Partial<ExtraCost>): Promise<ExtraCost> {
    return this.patch<ExtraCost>(`/api/extra-costs/${id}`, data);
  }

  async deleteExtraCost(id: string): Promise<void> {
    await this.delete(`/api/extra-costs/${id}`);
  }

  // Maintenances CRUD
  async getMaintenances(): Promise<Maintenance[]> {
    try {
      return await this.get<Maintenance[]>('/api/maintenances');
    } catch {
      return [];
    }
  }
  ...
  async createMaintenance(data: Partial<Maintenance>): Promise<Maintenance> {
    return this.post<Maintenance>('/api/maintenances', data);
  }

  async updateMaintenance(id: string, data: Partial<Maintenance>): Promise<Maintenance> {
    return this.patch<Maintenance>(`/api/maintenances/${id}`, data);
  }

  async deleteMaintenance(id: string): Promise<void> {
    await this.delete(`/api/maintenances/${id}`);
  }

  // Accounting Documents CRUD
  async getAccountingDocuments(): Promise<AccountingDocument[]> {
    try {
      return await this.get<AccountingDocument[]>('/api/accounting-documents');
    } catch {
      return [];
    }
  }

  async createAccountingDocument(data: Partial<AccountingDocument>): Promise<AccountingDocument> {
    return this.post<AccountingDocument>('/api/accounting-documents', data);
  }

  async updateAccountingDocument(id: string, data: Partial<AccountingDocument>): Promise<AccountingDocument> {
    return this.patch<AccountingDocument>(`/api/accounting-documents/${id}`, data);
  }

  async deleteAccountingDocument(id: string): Promise<void> {
    await this.delete(`/api/accounting-documents/${id}`);
  }
  ...
  async getEventReservations(eventId: string): Promise<TableReservation[]> {
    return this.get<TableReservation[]>(`/api/e4u/events/${eventId}/reservations`);
  }

  async getEventTableTypes(eventId: string): Promise<any[]> {
    return this.get<any[]>(`/api/e4u/events/${eventId}/table-types`);
  }

  async approveReservation(reservationId: string): Promise<any> {
    return this.post<any>(`/api/e4u/reservations/${reservationId}/approve`, {});
  }

  async rejectReservation(reservationId: string, reason?: string): Promise<any> {
    return this.post<any>(`/api/e4u/reservations/${reservationId}/reject`, { reason });
  }

  async getReservationGuests(reservationId: string): Promise<TableGuestEntry[]> {
    return this.get<TableGuestEntry[]>(`/api/e4u/reservations/${reservationId}/guests`);
  }

  async getEventGuestChangeRequests(eventId: string): Promise<GuestChangeRequest[]> {
    return this.get<GuestChangeRequest[]>(`/api/e4u/events/${eventId}/guest-change-requests`);
  }

  async getReservationGuestChangeRequests(reservationId: string): Promise<GuestChangeRequest[]> {
    return this.get<GuestChangeRequest[]>(`/api/e4u/reservations/${reservationId}/guest-change-requests`);
  }

  async createGuestChangeRequest(reservationId: string, data: {
    ...
    email?: string;
    notes?: string;
  }): Promise<GuestChangeRequest> {
    return this.post<GuestChangeRequest>(`/api/e4u/reservations/${reservationId}/guest-change-requests`, data);
  }

  async approveGuestChangeRequest(requestId: string): Promise<any> {
    return this.post<any>(`/api/e4u/guest-change-requests/${requestId}/approve`, {});
  }

  async rejectGuestChangeRequest(requestId: string, reason?: string): Promise<any> {
    return this.post<any>(`/api/e4u/guest-change-requests/${requestId}/reject`, { reason });
  }
  ...
  async getEventHubPRs(eventId: string): Promise<EventHubPR[]> {
    try {
      return await this.get<EventHubPR[]>(`/api/gestore/events/${eventId}/hub/prs`);
    } catch {
      return [];
    }
  }

  async getEventHubAccesses(eventId: string): Promise<EventHubAccess[]> {
    try {
      return await this.get<EventHubAccess[]>(`/api/gestore/events/${eventId}/hub/accesses`);
    } catch {
      return [];
    }
  }

  async getEventHubSales(eventId: string): Promise<EventHubSale[]> {
    try {
      return await this.get<EventHubSale[]>(`/api/gestore/events/${eventId}/hub/sales`);
    } catch {
      return [];
    }
  }

  async checkInGuest(eventId: string, guestId: string): Promise<void> {
    return this.post(`/api/gestore/events/${eventId}/guests/${guestId}/checkin`, {});
  }

  async addEventGuest(eventId: string, data: { name: string; email?: string; phone?: string; listId?: string }): Promise<void> {
    return this.post(`/api/gestore/events/${eventId}/guests`, data);
  }

  async bookEventTable(eventId: string, data: { tableId: string; customerName: string; guestCount: number; notes?: string }): Promise<void> {
    return this.post(`/api/gestore/events/${eventId}/tables/book`, data);
  }

  async releaseEventTable(eventId: string, tableId: string): Promise<void> {
    return this.post(`/api/gestore/events/${eventId}/tables/${tableId}/release`, {});
  }
...
export interface FixedCost {
  ...
  category: string;
  amount: string;
  frequency: 'monthly' | 'quarterly' | 'yearly' | 'per_event';
...
export interface AccountingDocument {
  id: string;
  name: string;
  type: 'invoice' | 'receipt' | 'contract' | 'permit' | 'other';
...
export interface TableReservation {
  ...
  tableTypePrice?: number;
  tableTypeMaxGuests?: number;
}

export interface TableGuestEntry {
  id: string;
...
export interface EventHubAccess {
  id: string;
  guestName: string;
  ticketType: string;
  checkinTime: string;
  checkpointName: string;
  method: 'qr' | 'manual' | 'list';
}

export interface EventHubSale {
  id: string;
  ticketType: string;
  customerName?: string;
  amount: number;
...
export interface UserFeatures {
  ...
  contabilitaEnabled: boolean;
  personaleEnabled: boolean;
  cassaEnabled: boolean;
  nightFileEnabled: boolean;
  siaeEnabled: boolean;
  scannerEnabled: boolean;
  prEnabled: boolean;
  prWalletEnabled: boolean;
  prReservationsEnabled: boolean;
  prPayoutsEnabled: boolean;
  prMultiCompanyEnabled: boolean;
  badgesEnabled: boolean;
  cassaBigliettiEnabled: boolean;
  templateEnabled: boolean;
  canCreateProducts: boolean;
  skipSiaeApproval: boolean;
