  ...
  useEffect(() => {
    const loadSavedToken = async () => {
      try {
        ...
        console.log('[Auth] Loading saved token from SecureStore...');
        if (savedToken) {
          api.setAuthToken(savedToken);
          console.log('[Auth] Token loaded and set:', savedToken.substring(0, 15) + '...');
        } else {
          console.log('[Auth] No saved token found in SecureStore');
        }
      } catch (error) {
  ...
  const login = async (identifier: string, password: string): Promise<{ role?: string }> => {
    try {
      ...
      console.log('[Auth] Login response received:', { 
        ...
        tokenPrefix: response.token ? response.token.substring(0, 15) : 'NONE'
      });
      ...
      if (response.user) {
        ...
        if (response.token) {
          await SecureStore.setItemAsync(AUTH_TOKEN_KEY, response.token);
          api.setAuthToken(response.token);
          console.log('[Auth] Token saved to SecureStore and set for API requests:', response.token.substring(0, 15) + '...');
        } else {
          console.log('[Auth] WARNING: No token received from server! PR features may not work.');
        }
