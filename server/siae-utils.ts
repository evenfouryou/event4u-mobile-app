...
export interface SiaeFileNameValidationResult {
  ...
  warnings: string[];
  parsedData?: {
    reportType: 'RMG' | 'RPM' | 'RCA' | 'LTA' | null; // Prefisso nome file
    date: string | null;
    systemCode: string | null; // NON presente nel nome file, solo nel Subject
...
export function validateSiaeFileName(fileName: string): SiaeFileNameValidationResult {
  ...
  if (!/^\d{4}$/.test(year)) {
    ...
    if (yearNum < 2000 || yearNum > 2100) {
      errors.push(`ANNO NON VALIDO: L'anno deve essere tra 2000 e 2100. Trovato: ${yearNum}`);
    }
  }
...
export function generateSiaeAttachmentName(
  ...
  console.log(`[SIAE-UTILS] generateSiaeAttachmentName: type=${reportType}, result=${result} (formato breve Allegato C 1.4.1)`);
  ...
  if (reportType === 'rca') {
    ...
    if (reportType === 'mensile') {
      ...
      if (meseMatch) {
        ...
        // NOTA: Rimossa validazione "mese non concluso" (2026-01-22)
        // La SIAE potrebbe accettare RPM anche durante il mese corrente
